...
console.log(getAuth0ClientId(process.env.VERCEL_ENV));
...
// ManagementClientを介してManagement APIトークンを自動的に取得するには
// パラメータclientId、clientSecret（Non Interactive Clientを使用）、オプションでscopeを指定します
// 舞台裏では、Client Credentials Grantがアクセストークンの取得に使用され
// デフォルトでは、返されたexpires_inの値の間、キャッシュされます
const management = new ManagementClient({
  token: TEST_ACCESS_TOKEN,
  domain: process.env.AUTH0_DOMAIN || 'dev-dt8p9ro2.us.auth0.com',
  clientId: getAuth0ClientId(process.env.VERCEL_ENV),
  clientSecret: '4h_F-vUcsS9kgoUAnxw4JfMwKxg8S_xY50sVVhsEj2ZFeoOhKz5O0nkmcMmr0WzA',
  scope: 'update:users',
});

const updateAuth0User = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  try {
    ...
    await management.updateUser({ id: params[0] }, { nickname: params[1], picture: req.body });
    res.status(200).end();
  } catch (error) {
